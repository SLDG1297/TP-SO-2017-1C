...
void menuConsola(){
	printf("Bienvenido a la consola de la memoria porfavor ingrese una de la siguientes opciones");
		printf("1.Modificar modificarRetardo de operacion en la memoria");
		printf("2.Reporte de contenido");
		printf("3.Limpiar la memoria cache - FLUSH");
		printf("4.Indicar tamano");
		printf("5.Salir de la consola");
}

void menuDump(){
	printf("Reportes completos de: ");
	printf("1.La memoria cache");
	printf("2.La tabla de paginas y los procesos activos");
	printf("3.Datos almacenados en todos los procesos o en uno en particular");
}
...
void verContenidoDeMemoria(){
	...
	while(c<_frames){
		...
		if(adm.pid!=LIBRE&&adm.pid!=OCUPADO_x_STR){
			...
			log_info(archivoLog,"\nPID = %d \nPAGINA = %d \nFRAME = %d \nCONTENIDO = %s\n",adm.pid,adm.nroPagina,adm.frame,(char *) contenido);
...
void flush() {
	int opcion=0;
	printf("Esta seguro de que desea limpiar el contenido de la cache.\n1.Si\n2.No");
	scanf("%d",opcion);
	if(opcion==1){
