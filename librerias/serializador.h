...
void* 	recibirDatos(int socket); // Para recibir datos de otro proceso. Hace un malloc.
...
void enviarDatos(int socket, void* datos, size_t tamanioDatos)
{
	...
	void* tamanioDatosAux = &tamanioDatos; // Para que send no hinche las pelotas...

	// Indica el tamaño de lo que se envía a continuación. (Header)
	verificador = send(socket, tamanioDatosAux, sizeof(int), 0);
...
void* recibirDatos(int socket)
{
	...
	void* tamanioAux; // Para que recv no hinche las pelotas...
